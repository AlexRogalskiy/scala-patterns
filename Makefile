...
OKTETO_IMAGE ?= okteto/styled-scala-patterns
DOCKER_IMAGE ?= alexanderr/styled-scala-patterns
TAG ?= latest
...
.PHONY: helm-start
helm-start:
	helm upgrade --install backend-scala-patterns -f charts/values.yaml --create-namespace --namespace webapp charts
